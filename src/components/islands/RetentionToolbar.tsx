...
export const RetentionToolbar: FunctionalComponent<RetentionToolbarProps> = ({
    ...
    const componentId = id ?? `retention-toolbar-${Math.random().toString(36).slice(2, 10)}`;
    ...
    const prevParts = animating ? getParts(options[animating.prev]) : { number: '', unit: '' };
    const currParts = animating ? getParts(options[animating.curr]) : getParts(options[currentIndex]);
                ...
                <div class="retention-value-container">
                    {animating ? (
                        <>
                            ...
                            >
                                <span class={`retention-number animate-${animating.direction}-exit`} style={{ whiteSpace: 'pre' }}>
                                    {valuePrefix}{prevParts.number}
                                </span>
                                <span 
                            ...
                            >
                                <span class={`retention-number animate-${animating.direction}-enter`} style={{ whiteSpace: 'pre' }}>
                                    {valuePrefix}{currParts.number}
                                </span>
                                <span class={`retention-unit ${unitChanged ? `animate-${animating.direction}-enter` : ''}`}>
                                    {currParts.unit}
                                </span>
                            </div>
                        </>
